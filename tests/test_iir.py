...
def iir_sim(impl,t_coef, t_in, t_out,seq,target='build/fir',do_cosim=False):
...
def test_iir_direct(tmpdir, impl, seed, do_cosim):
    ...
    log.info(f'Running test_fir_direct tmpdir: {tmpdir}, impl: {impl}, seed: {seed}')
...
def test_iir_random_type(impl, seed, do_cosim):
    ...
    log.info(f'{__name__} FIR input type t_b: {ftype} min: {ftype.fmin}, max: {ftype.fmax}')
